...
### 2.5 基于锁的协议
**最基础**：两阶段锁协议（Two-Phase Locking, 2PL）
- 增长阶段  
    刚开始获取锁时，只能获取，不能释放
- 缩减阶段  
    结束时释放锁时，只能释放，不能获取
...
- **Rigorous two-phase locking (强两阶段封锁)**   
    所有的锁都只能在commit后边释放
### 2.6 死锁  
就是两个事务互相持有对方需要的锁，导致阻塞。为了解决死锁，必须回滚并释放其锁。如图，都在等待对方释放锁。  

![alt text](image/死锁.png)

两阶段锁协议，包括其变体，无法避免死锁(常考题)
...
**(1)锁竞争导致的饥饿**  
- 共享锁（S锁）过多，排他锁（X锁）无法获取
- 短事务频繁获取锁，导致长事务一直无法执行。

**(2) 调度策略导致的饥饿**
- 由于死锁，同一事务被反复回滚
## 3. 死锁
### 死锁的预防
- **预声明（Predeclaration）**，在事务开始前锁定所有资源，这可能导致很多数据被锁长时间无法使用，造成浪费
- **基于图的协议（Graph-Based Protocol）**：对数据项施加 偏序（partial ordering），要求事务必须按规定的顺序加锁。
- **基于时间戳的协议(timestamp-ordering protocol)**:每笔交易在进入系统时都会被分配一个时间戳,通过时间戳确定并发执行的顺序

Wait-Die（等待-死亡）:较老的事务可以等待较新的事务释放锁。  
Wound-Wait（伤害-等待）——抢占式（Preemptive）:较老的事务如果请求的资源被较新的事务持有，会强制回滚（wound）较新的事务（抢占资源）。较新的事务如果请求的资源被较老的事务持有，可以等待。
### 死锁的检测
死锁可以用 等待图（Wait-for Graph, WFG）表示为一个有向图 
$G=(V,E)G=(V,E)$ ：系统处于死锁状态当且仅当等待图中存在环（cycle）。

### 死锁的恢复
选择一个“牺牲者”的事务，终止事务(Kill)，并且回滚。可以全部回滚，也可以部分回滚(只回滚到足以解除死锁的状态，而不是完全终止事务，这样可以减少开销。)
