/* 2351136 李盛鹏 大数据 */
/* 2353572 汪君哲 2351582 程亦诚 2352848 许慧琳 */
...
#endif
void printTestResult(const string& testName, const string& expected, const string& actual)
{
	cout << "测试: " << testName << endl;
	cout << "预期输出: " << expected << endl;
	cout << "实际输出: " << actual << endl;
	if (expected == actual)
	{
		cout << "结果: 通过" << endl;
	}
...
int main()
{
	if (1)
	{
		...
		TStringAdv ts1;
		ts1.assign("Hello");
		printTestResult("ts1.assign(\"Hello\")", "Hello", ts1.c_str());

		TStringAdv ts2;
		ts2.assign(ts1);
		printTestResult("ts2.assign(ts1)", "Hello", ts2.c_str());

		// 测试 append 函数
		ts1.append(" World");
		printTestResult("ts1.append(\" World\")", "Hello World", ts1.c_str());

		ts1.append('!');
		printTestResult("ts1.append('!')", "Hello World!", ts1.c_str());

		ts1.append(ts2);
		printTestResult("ts1.append(ts2)", "Hello World!Hello", ts1.c_str());

		// 测试 insert 函数
		ts1.insert("INSERT", 6);
		printTestResult("ts1.insert(\"INSERT\", 6)", "HelloINSERT World!Hello", ts1.c_str());

		ts1.insert('X', 1);
		printTestResult("ts1.insert('X', 1)", "XHelloINSERT World!Hello", ts1.c_str());

		ts1.insert(ts2, 10);
		printTestResult("ts1.insert(ts2, 10)", "XHelloINSHelloERT World!Hello", ts1.c_str());

		// 测试 erase 函数
		ts1.erase("HELLO");
		printTestResult("ts1.erase(\"HELLO\")", "XHelloINSHelloERT World!Hello", ts1.c_str());

		ts1.erase('X');
		printTestResult("ts1.erase('X')", "HelloINSHelloERT World!Hello", ts1.c_str());

		ts1.erase(ts2);
		printTestResult("ts1.erase(ts2)", "INSHelloERT World!Hello", ts1.c_str());

		// 测试 substr 函数
		TStringAdv sub = ts1.substr(6, 7);
		printTestResult("ts1.substr(6, 7)", "lloERT ", sub.c_str());

		TStringAdv sub2 = ts1.substr(15);
		printTestResult("ts1.substr(15)", "rld!Hello", sub2.c_str());

		TStringAdv sub3 = ts1.substr(20);
		printTestResult("ts1.substr(20)", "ello", sub3.c_str());

		// 测试 at 函数
		char c = ts1.at(1);
		printTestResult("ts1.at(1)", "N", string(1, c));

		// 测试边界条件和异常情况
		TStringAdv empty;
		empty.insert("TEST", 1);
		printTestResult("empty.insert(\"TEST\", 1)", "TEST", empty.c_str());

		empty.erase("TEST");
		printTestResult("empty.erase(\"TEST\")", "", empty.c_str());

		empty.append("");
		printTestResult("empty.append(\"\")", "", empty.c_str());

		empty.insert("", 1);
		printTestResult("empty.insert(\"\", 1)", "", empty.c_str());

		empty.erase('X');
		printTestResult("empty.erase('X')", "", empty.c_str());

		// 测试非法 pos 情况
		ts1.insert("INVALID", 0);
		printTestResult("ts1.insert(\"INVALID\", 0)", "INSHelloERT World!Hello", ts1.c_str());

		ts1.insert("INVALID", -1);
		printTestResult("ts1.insert(\"INVALID\", -1)", "INSHelloERT World!Hello", ts1.c_str());

		TStringAdv a("ABCDE");
		printTestResult("TStringAdv a(\"ABCDE\")", "ABCDE", a.c_str());
		a.insert("123", 3); // 插入到位置3，期望 "AB123CDE"
		printTestResult("a.insert(\"123\", 3)", "AB123CDE", a.c_str());

		a.insert('\0', 4); // 插入 '\0' 到位置4，期望 "XYZ"
		printTestResult("a.insert('\0', 4)", "AB1", a.c_str());


		cout << "我的测试完成，下面是原16作业的测试" << endl;
		cout << "按下空格测试" << endl;
		wait_for_enter();
