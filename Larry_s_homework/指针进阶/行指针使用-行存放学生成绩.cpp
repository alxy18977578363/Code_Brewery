/* 2351136 李盛鹏 大数据 */
#include <iostream>
#include <iomanip>
using namespace std;

#define STUDENT_NUM	4
#define SCORE_NUM	5

/* --- 不允许定义任何形式的全局变量 --- */

/***************************************************************************
  函数名称：average
  功    能：求第一门课的平均分
  输入参数：
  返 回 值：
  说    明：相关输出都放在函数中即可
***************************************************************************/
void average(int(*score)[SCORE_NUM])
{
    /* 函数定义语句部分：
       1、本函数中仅允许定义 1个简单变量 + 1个指针变量 */

       /* 函数执行语句部分：
          1、不允许再定义任何类型的变量，包括 for (int i=0;...）等形式定义变量
          2、循环变量必须是指针变量，后续语句中不允许出现[]形式访问数组
             不允许：int a[10], i;
                     for(i=0; i<10; i++)
                         cout << a[i];
             允许  ：int a[10], p;
                     for(p=a; p<a+10; p++)
                         cout << *p;          */
    // 总和和一个指针变量
    double sum = 0;
    int(*p)[SCORE_NUM] = score;
   

    for (; p < score + STUDENT_NUM; p++)
    {
        sum += (*p)[0];
    }

    cout << sum / STUDENT_NUM << endl;



}

/***************************************************************************
  函数名称：fail(int(*score)[SCORE_NUM])
  功    能：找出有两门以上课程不及格的学生
  输入参数：int(*score)[SCORE_NUM]
  返 回 值：
  说    明：相关输出都放在函数中即可
***************************************************************************/
void fail(int(*score)[SCORE_NUM])
{
    /* 函数定义语句部分：
       1、本函数中仅允许定义 2个简单变量 + 1个行指针变量 + 1个简单指针变量 */

       /* 函数执行语句部分（要求同average）*/
  
    int(*hang)[SCORE_NUM] = score;
    int* b = *hang;
    for (; hang < score + STUDENT_NUM; hang++)
    {
        int count = 0;
        double sum = 0;
        for (b=*hang; b < *hang + SCORE_NUM; b++)        // 相当于a+i，表示第i个元素
        {
            if (*b < 60)
                count++;
        }

        if (count >= 2)
        {
            cout << "No：" << (hang - score) + 1 << " ";

            // 输出成绩
            for (b = *hang; b < *hang + SCORE_NUM; b++)
            {
                cout << *b << " ";
                sum += *b;
            }

            cout << "平均：" << sum / SCORE_NUM << endl;
        }

    }
    

}

/***************************************************************************
  函数名称：good
  功    能：找出平均成绩在90分以上或全部成绩在85分以上的学生
  输入参数：int(*score)[SCORE_NUM]
  返 回 值：void
  说    明：我将通过一个行指针找出一个个学生，通过元素指针在对应行读取成绩进行计算
***************************************************************************/
void good(int(*score)[SCORE_NUM])
{
    /* 函数定义语句部分：
       1、本函数中仅允许定义 2个简单变量 + 1个行指针变量 + 1个简单指针变量 */

       /* 函数执行语句部分（要求同average）*/
    int(*hang)[SCORE_NUM] = score;
    int* p = *hang;

    for (; hang < score + STUDENT_NUM; hang++)
    {
        double sum = 0;
        int num = 0;
        for (p = *hang; p < *hang + SCORE_NUM; p++)
        {
            num += (*p >= 85);
            sum += *p;
        }

        if (sum / SCORE_NUM >= 90 || num == SCORE_NUM)
        {
            cout << "No：" << hang - score + 1 << " ";
            for (p = *hang; p < *hang + SCORE_NUM; p++)
            {
                cout << *p << " ";
            }

            cout << "平均：" << sum / SCORE_NUM << endl;
        }
    }

}

/***************************************************************************
  函数名称：main()
  功    能：
  输入参数：
  返 回 值：
  说    明：
***************************************************************************/
int main()
{
    int a[STUDENT_NUM][SCORE_NUM] = {
        {91, 81, 71, 61, 51},	//第1个学生的1-5门课的成绩
        {92, 82, 72, 32, 52},   //第2个学生的1-5门课的成绩
        {93, 83, 99, 80, 95},   //第3个学生的1-5门课的成绩
        {97, 85, 87, 91, 88} }; //第4个学生的1-5门课的成绩

    /* 除上面的预置数组外，本函数中仅允许定义 1个行指针变量 + 1个简单指针变量 */
    int(*hang)[SCORE_NUM] = a;
    int* elem = &a[0][0];
    /* 函数执行语句部分（要求同average）*/

    cout << "初始信息：" << endl;
    for (; hang < a + STUDENT_NUM; hang++)
    {
        cout << "No." << (hang - a)+1  << "的1-5门课成绩：";
        for (elem = *hang; elem < *hang + SCORE_NUM; elem++)
        {
            cout << *elem << " ";
        }
        cout << endl;
    }

    cout << endl;
    // 第一个函数的展示
    cout << "第1门课平均分：";
    average(a);
    cout << endl;
    // 第二个函数的展示
    cout << "2门以上不及格的学生：" << endl;
    fail(a);
    cout << endl;

    // 第三个函数的展示
    cout << "平均90以上或全部85以上的学生：" << endl;
    good(a);

    return 0;
}