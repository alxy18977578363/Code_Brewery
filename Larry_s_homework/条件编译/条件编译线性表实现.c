...
Status PriorElem(sqlist L, ElemType cur_e, ElemType* pre_e, Status(*compare)(ElemType e1, ElemType e2))
{
    ...
    /* 循环比较整个线性表 */
    while (i <= L.length && (*compare)(cur_e, *p) == FALSE)
    {
        i++;
        p++;
    }
...
/* 清除线性表(已初始化，清除内容，将表格变回原来的空间) */
Status ClearList1(sqlist* L)
{
    /* 不管是普通类型的指针还是结构体类型的指针，都要释放二次空间 */
#if defined (ELEMTYPE_IS_CHAR_P) || defined (ELEMTYPE_IS_STRUCT_STUDENT_P)
    int i;
...
#endif
    /* 如果曾经扩展过空间，要变成原来的空间 */
    if (L->listsize > LIST_INIT_SIZE)       // 超过原来的空间
    {
        ElemType* newbase;
        newbase = (ElemType*)realloc(L->elem, LIST_INIT_SIZE * sizeof(ElemType));
        if (!newbase)
            return LOVERFLOW;        // 将线性表大小变回原先的空间
    ...
    L->length = 0;                  // 不管是否拓展过空间，都应该清除，虽然底层的数组没有清除，但是外部已经不能再调用

    return OK;
}

/* 在指定位置前插入一个新元素 */
Status ListInsert1(sqlist* L, ElemType cur_e, ElemType e,
    Status(*compare)(ElemType e1, ElemType e2))
{
    ElemType* p, * q; //如果是算法，一般可以省略，程序不能

    /* 寻找我需要的元素的位置 */
    ElemType* Travel_p = L->elem + L->length - 1;
    int i = L->length;                              // 从后往回找
    while (i >= 1 && (*compare)(*Travel_p--, cur_e) == FALSE)          // Travel_p会先取值再向前一位，找到或者遍历结束才能退出循环
        i--;

    /* 无法获取相应字符，相当于遍历完了整个线性表 */
    if (i < 1)
    {
        return ERROR;
    }
    ...
    for (p = &(L->elem[L->length - 1]); p >= q; --p)
        ...
        memcpy(p + 1, p, sizeof(ElemType));	//不能用strcpy
#else	//int、double、char指针、struct student指针都是直接赋值
        * (p + 1) = *p;                     // 相当于elem[p.index+1]=elem[p.index]
#endif
...
2.只删除第一个
=====================================*/
Status ListDelete1(sqlist* L, ElemType cur_e,
    Status(*compare)(ElemType e1, ElemType e2))
{
    ElemType* p, * q; //如果是算法，一般可以省略，程序不能

    /* 寻找我需要的元素的位置 */
    ElemType* Travel_p = &(L->elem[0]);
    int i = 1;                              // 从前往后找
    while (i <= L->length && (*compare)(*Travel_p++, cur_e) == FALSE)          // Travel_p会先取值再向前一位，找到或者遍历结束才能退出循环
        i++;
    ...
    L->length--;	//长度-1
    /* 空间增缩,让size不超过length+5(length为105/115等时) */
    if (L->length < L->listsize - LISTINCREMENT - SHRINK_FACTOR && L->listsize != LIST_INIT_SIZE)
    {
        int new_upper_limit = ((L->length / 10) + 1) * 10;
        L->listsize = new_upper_limit;
    }
...
3.补充空间增缩功能
=====================================*/
Status ListDelete2(sqlist* L, ElemType cur_e,
    Status(*compare)(ElemType e1, ElemType e2))
{
    ElemType* p, * q; //如果是算法，一般可以省略，程序不能
    bool is_found = false;
    ...
    /* 寻找我需要的元素的位置 */
    ElemType* Travel_p = L->elem;                            // 从前往后找                   
    while (Travel_p <= &(L->elem[L->length - 1]))
    {
        /* 寻找到该消除的元素 */
        if ((*compare)(*Travel_p, cur_e) == TRUE)
        {
            is_found = true;
            p = Travel_p; 		//指向第i个元素
