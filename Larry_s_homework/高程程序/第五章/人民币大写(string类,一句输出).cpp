...
int main()
{
	...
	if (shiyi) {
		Change(shiyi);
		result+= connection[2 * 2];
		result+= connection[2 * 2 + 1];
	}
	...
	if (yi + shiyi != 0) {
		result+= connection[4 * 2];
		result+= connection[4 * 2 + 1];
	}
	...
	if (!qianwan && _qianwan) {
		...
		Change(qianwan);
		result+= connection[0 * 2];
		result+= connection[0 * 2 + 1];
	}
	...
	if (!baiwan && _baiwan) {
		...
		Change(baiwan);
		result+= connection[1 * 2];
		result+= connection[1 * 2 + 1];
	}
	...
	if (qianwan + baiwan + shiwan + wan != 0) {
		result+= connection[3 * 2];
		result+= connection[3 * 2 + 1];
	}
	...
	if (shiyi + yi + qianwan + baiwan + shiwan + wan + qian + bai + shi + yuan != 0 || x == 0) {
		result+= connection[5 * 2];
		result+= connection[5 * 2 + 1];
	}
	...
	if (jiao) {
		Change(jiao);
		result+= connection[6 * 2];
		result+= connection[6 * 2 + 1];
	}
	//整
	if (fen == 0) {
		result+= connection[8 * 2];
		result+= connection[8 * 2 + 1];
	}

	//分
	if (fen) {
		Change(fen);
		result+= connection[7 * 2];
		result+= connection[7 * 2 + 1];
	}
