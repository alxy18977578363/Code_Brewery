...
public:
	...
	operator Teacher();				// 类型转化函数
	Student& operator=(const class Teacher& te);		// 转换构造函数
...
{
	num = 2150000 + (te.num % 1000); // 学号 = 2150 + 工号后三位

	strncpy(name, "学生", sizeof(name) - 1); // 姓名转换为"学生"
	strncpy(name + 4, te.name + 4, sizeof(name) - 5); // 后续字符不变
	name[sizeof(name) - 1] = '\0'; // 确保字符串以'\0'结尾

	sex = te.sex; // 性别原样转换

	strncpy(addr, te.addr, sizeof(addr) - 1); // 复制地址
	strcat(addr, "101室"); // 地址后加"101室"
	addr[sizeof(addr) - 1] = '\0'; // 确保字符串以'\0'结尾
...
Student::operator Teacher()
{
	...
	temp.num = 21000 + (num % 1000);

	strncpy(temp.name, "教师", sizeof(temp.name) - 1); // 姓名转换为"教师"
	strncpy(temp.name + 4, name + 4, sizeof(temp.name) - 5); // 后续字符不变
	temp.name[sizeof(temp.name) - 1] = '\0'; // 确保字符串以'\0'结尾

	temp.sex = sex;

	strncpy(temp.addr, addr, sizeof(temp.addr) - 1); // 复制地址
	strcat(temp.addr, "电信学院"); // 地址后加"电信学院"
	temp.addr[sizeof(temp.addr) - 1] = '\0'; // 确保字符串以'\0'结尾
