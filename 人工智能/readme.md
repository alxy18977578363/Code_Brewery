...
<details>
<summary><b>第二章 知识表达与推理</b></summary>

## 1. **逻辑**
从一个或若干前提出发，能否找到一个论证或者推理，能推出结论，在前提和结论之
间搭建 **逻辑结构** 的桥梁。 这就是逻辑的内涵。
...
命题逻辑中有**原子命题**和**复合命题**。
- **原子命题**： 不可拆分的命题
- **复合命题**：复合命题用命题联结词(与或非，蕴含，等价)将原子命题相连。
- **真值表**
...
### 2.3 谓词逻辑
因为命题逻辑不能处理语句中的个体和总体的关系，所以引入**谓词逻辑**。
...
值得一提的一阶谓词逻辑和高阶谓词逻辑的关系

- **一阶逻辑**：  
  - 量词（ $\forall, \exists$ ）只能作用于**个体变量**（如 $x, y$ ）。  
  - **示例**：  
...
否定既要作用在量词上，又要作用在谓词上，才能表达出全盘否定的含义。
...
**解释**：  
- $\exists x \, \forall y \, \text{Loves}(x,y)$ ：**“存在一个人爱所有人”**（如“上帝爱世人”）。  
- $\forall y \, \exists x \, \text{Loves}(x,y)$ ：**“每个人都至少被一个人爱”**（如“每人都有母亲”）。  

前者强调同一个 x 对所有 y 成立，后者允许对每个 y 选择不同的 x 。
### 量词分配律的等价关系分析
...
---
## 3. **归结原理及应用**(重点)
作用是推导一个结论是否成立，用的方法则是去除子句集中的文字来验证子句集的不可满足性。

给你一个合取范式：
$$A \land (B \lor C) \land (D \lor E) \land (F \lor G)$$

- **子句**：用 $\land$ 连起来的每一个整体，都叫做一个子句。
- **子句集**：这些子句的集合称为子句集
- **文字**，子句内部每个独立的谓词，不管是真是假，都叫文字。
- **NIL**：如果子句中没有文字，说明无法实现，被称为**NIL**

而想要运用归结原理一共需要两步：
1. **求子句集**
2. **合并**：通过变量替换使两个表达式相同的过程（如{a/x} 表示用a替换x）

将要证明的结果，写其相反的式子，然后用归结原理的结果必然是**NIL**，证明假设错误，结果成立。

### 3.1 **化为子句集**  
化为子句集，在某种意义上，就是化为合取范式，但是由于是在谓词逻辑上，所以复杂一些。
#### 消去蕴含
通过蕴含等值式 $\alpha \rightarrow \beta \equiv \neg \alpha \lor \beta$ ，将 -> 去掉
#### 否定内移
应用德摩根定律, 使得外层的 $\neg$ 移动到谓词旁边
#### 变量标准化
避免重名导致意思混淆  
#### 消存在量词（Skolem化）  
这里是消除 $\exists$ , 有两种情况：
1. 如果 $\exists x$ 不是嵌套在任何的 $\forall$ 内，直接删去，将x 用常数A代替。
2. 如果 $\exists x$ 嵌套在 $\forall$ 内，去掉 $\exists$ ,用 **Skolem函数（f(y)）** 代替原变量x ，但是要注意，这个f要是一个未出现过的函数。

#### 化为前束形后删除全称量词  
把 $\forall$ 移到前面后，就可以去掉了，因为不影响了。
#### 得到子句集
如 $\{P(x), \neg Q(y)\}$

---
### 3.2 **MGU 最简合一者**
在获取了子句集后，需要对内部子句进行两两相交合并。由于有些文字，它函数相同，但是变量不一样，所以需要用MGU 来替换变量，达到合一效果。

$\{A/ x\}$ ，从左往右读，这个叫以A换掉x。值得注意的是：**常量可以换变量，变量不能换常量。**

---
### 归结原理结果
只要最后子句两两相交，去掉所有文字，剩下NIL，就可以证明结论成立。

---
## 4. **概率推理**(重点)
这个知识点的引入，主要是一个系统时常会出现三种情况：  

1. **惰性**: 系统有足够的前提和结论，但是为了节省算力，只列举部分。
2. **无知**: 缺乏推理需要的数据，或者知识。
3. **不完备性**: 逻辑系统本身的表达能力不足，无法解决特定问题。

**解决办法**
1. 惰性可通过优化算法缓解（如启发式搜索）。
2. 无知需补充知识（如数据采集或理论研究）。
3. 不完备性需升级系统（如从命题逻辑转向高阶逻辑）。

我们常借助概率图模型来解决概率推理问题，这类图一般分为**贝叶斯网络（Bayesian Network）**和**马尔可夫网络**

### 4.1 贝叶斯网络
一种基于有向无环图（DAG）的概率图模型，表示随机变量间的条件依赖关系。

- **随机变量**：以大写字母作为开头的变量，其多种的取值表现现实情况多样性。

描述变量联合分布或者数据生成机制的模型，被称为 “贝叶斯网络”（Bayesian network）。

#### 4.2 一些概念
- **先验概率（无条件概率）**
- **条件概率**
- **联合概率分布**：多个变量取值的所有组合的概率矩阵，由乘法原理取得
#### 4.3 贝叶斯网络的法则
- **乘法法则**
$$P(A \land B) = P(B \mid A) P(A) = P(A \mid B) P(B)$$
- **链式法则**
$$P(X_1, X_2, \dots, X_n) = \prod_{i=1}^n P(X_i \mid X1 \dots X_{i-i})$$

- **条件独立性**：  
  给定父节点，节点与其非后代节点独立。数学表达：  
$$P(X \mid \text{Parents}(X), Y) = P(X \mid \text{Parents}(X)) \quad (Y \text{为非后代})$$
...
<details>
<summary><b>第三章 搜索探寻与问题</b></summary>

# 搜索探寻与问题
## 1.搜索的形式化描述
用五个元素来描述搜索问题：状态、动作、状态转移、路径/代价、目标测试

- **状态**：当前的环境  
- **动作**：从一个状态转为另一个所需的操作  
- **状态转移**：从一个状态转为另一个  
- **目标测试**：判断当前状态是否为目标状态 
- **路径/代价**：状态序列就是路径，代价是路径中的决策点
- **目标测试**：用于判断状态𝐬是否为目标状态

**一个模型**
- 初始状态
- ACTIONS(s) 表示状态s下的可行行动
- RESULT(s，a) 表示在s下进行a后的结果
- 目标状态集，里面存储目标，每次行动都要检查是否s已经到目标
- c(s, a, s’)，表示从s到s'的代价

---
## 2. 搜索树的构建
树的根结点为初始状态，不断地扩展边界节点，找每个节点可到达的节点，绘制出一棵树。
...
前两个在乎的是，是否能完成题目要求。后面两个在乎的是，这个算法的好坏。

### 3.1 盲目搜索
没有任何信息，只能从初始节点开始，生成后继结点，判断是否到目标。
...
### 3.2 启发式搜索
启发式搜索的判断依据是评价函数 $f(n) = g(n) + h(n)$ 
- $g(n)$ "Generated cost" ，从起点到n的实际距离
- $h(n)$ "Heuristic"，表示从n到目标的理想距离(相对于实际距离)
- $f(n)$ 表示经过点n到目标的最小距离，这个值越小，越能纳入考虑范围。

如果 $g(n)$ 越大，则更看重已经探索的距离，则倾向于广度优先搜索。如果 $h(n)$ 越大，则更看重启发探索。
...
#### 注意的一点
启发式搜索算法都是基于纸上的演算，并非真正落于实际，所以，我们是可以走回头路的，向前的拓展节点不会因为你选择了某一步而消失，而是仍然作为你可拓展的节点，意思就是可以"悔棋"。
#### 3.2.1 贪婪最佳优先搜索
$$f(n) = h(n)$$
只关注怎么走距离终点最近，不考虑其它，可能会局部最优。
- 完备性 否–会陷入死循环，除非采用了排除环路的剪枝方法
- 最优性 否
- 时间复杂度 $O(b^m)$ , b 是分支数，m是深度
- 空间复杂度 $O(b^m)$


#### 3.2.1 A* 算法
$$f(n) = g(n) + h(n)$$
A* 这要比贪婪多考虑一个从起点到n点处实际行走距离。

#### A*可容性
意思就是我当前决策的 $h_{理想}(n) < h_{真实}(n)$ 

每一步决策的剩余距离都比真实剩余距离要小，所以是可容的。
#### A* 一致性
$$h(n) ≤ c(n,a,n') + h(n')$$

![alt text](image/A星一致性.png)
一致性强于可容性，满足一致性条件的启发函数一定满足可容性条件。

#### h(n)的挑选
因为h(n) 是自己设计的，所以可能会出现三种情况：
- $h(n)<h_{实际}(n)$ , 这个结果好，但是拓展节点多。
- $h(n)=h_{实际}(n)$ , 这个最好，直接朝着最优目标走。
- $h(n)>h_{实际}(n)$ , 这个结果不好，会误导搜索。
...
## 4. 对抗搜索
### 4.1 博弈论
竞争环境中决策主体（玩家）的最优策略与稳定局势，它是冯·诺伊曼与摩根斯特恩《博弈论与经济行为》（1944）提到的一个概念

- **核心要素**：  
    - **玩家（Player）**：决策主体。  
    - **策略（Strategy）**：纯策略（确定行动）与混合策略（概率分布）。  
    - **收益（Payoff）**：局势下的利益，混合策略下为期望收益。  
    - **规则（Rule）**：行动顺序、信息获取等。  

#### **博弈分类**  
- **合作 vs. 非合作**：是否允许联盟。  
- **静态 vs. 动态**：决策是否同时或顺序。  
- **完全信息 vs. 不完全信息**：玩家是否知晓全部信息。  

这里主要研究的是**双人、完备信息、零和（如象棋、围棋）**。  

#### **博弈形式化描述**  
初始状态 $S_0$ 、玩家函数 $PLAYER(s)$ 、合法动作集 $ACTIONS(s)$ 、转移模型 $RESULT(s,a)$ 、终止测试 $TERMINAL-TEST(s)$ 、效用函数 $UTILITY(s,p)$ 。  

#### **博弈树与解图**  
**MAX（我方）与 MIN（对手）**：交替行动，MAX最大化收益，MIN最小化收益。  
- **与或图表示**：要想赢，那MAX操作后的每一个MIN都不能使它赢，因此MAX节点之间用 **与** 来相连，而MIN操作后，后面的MAX节点只要有一种能赢即可，因此MIN节点间用 **或** 相连。
- **解图**：代表完整博弈策略，复杂博弈中难以穷举。因此这种思想不被采纳


### 4.2 **极小极大化算法（Minimax Algorithm）**
**核心思想**  
模拟人类下棋思维：假设对手最优回应，选择对己最有利的走步。  
- **静态估计函数 f(p)**：  
    - 对MAX有利为正，对MIN有利为负，终止状态为 $+\infty$（MAX赢）、 $-\infty$（MIN赢）或0（平局）。  

**算法步骤**  
- **第一阶段**：生成规定深度的博弈树，计算端节点静态估值。  
- **第二阶段**：自底向上倒推值，MAX取子节点最大值，MIN取最小值,这里求的是初始节点的静态估值。  
- **局限性**：时间复杂度 $O(b^m)$ ，空间复杂度 $O(bm)$ （ $b$ 为分支因子，$m$ 为深度）,只是一个大概的估计，并非真的b叉树。  

#### **三、α-β 剪枝（Alpha-Beta Pruning）**
通过剪枝减少搜索节点，不影响最终结果。    
每个节点都有一个下限α和上限β，当α大于β时，减去该节点和后代
- **α值**：MAX节点的当前下限（至少能获得的收益）。  
- **β值**：MIN节点的当前上限（至多能获得的收益）。 

**算法步骤**  
1. 设置根节点为 $α=−\infty$ ， $β=+\infty$ 。
2. 向下传递，先左子树后父节点后右子树，传递范围。
3. min层更新β，β = min(本层的值，下一层的α,下一层的β)，max层更新α，α = max(本层的值，下一层的α,下一层的β)。

**性能优化**  
   - 好的走步顺序可将时间复杂度降至 $O(b^{m/2})$ 。  
   - 实际实现：深度优先搜索，动态生成节点并剪枝。  
...
### 6.1 Softmax
$$a_i = \frac{e^{z_i}}{\sum_{j=1}^{K} e^{z_j}} \quad \text{其中} \quad i = 1, 2, \dots, K$$
...
$$loss(a_{1},\ldots, a_{N},y) = -\log a_{i} , \text{ if } y=i$$
