...
- **时序系统及微操作命令产生部件**
## 2. 运算器
- **算术逻辑单元ALU**：完成算术运算和逻辑运算
- **通用寄存器组**：存放操作数
- **暂存寄存器**：暂存从主存读来的数据，不放在通用寄存器中，防止破坏原先内容
- **程序状态字寄存器PSW**：保存溢出标志(OP),符号标志(SF),零标志(ZF),进位标志(CF)
- **移位器**：移位运算
- **计数器**：控制乘除运算的步骤
## 3. 控制器
### 3.1 控制器的功能
...
- **执行指令**：以 **“操作命令”** 和 **“操作数地址”** 形成操作信号控制序列，控制运算器，存储器及I/O设备完成操作。
- **中断处理**：控制总线及输入输出，处理异常情况(如掉电)和特殊请求(如打印一行字符)
### 3.2 控制器的组成
- **程序计数器PC**：指出下一条指令的存放地址
- **指令寄存器IR**：保存当前指令
- **指令译码器**：只对操作码字段进行译码，然后告诉微操作信号发生器
- **微操作信号发生器**：根据IR，PSW和时序信号，产生各种控制信号，由组合逻辑和存储逻辑两种
- **时钟脉冲(CP)**
- **时序信号发生器**：产生各种时序信号，由时钟(Clock)分频得到
- **存储器地址寄存器MAR**：存放要访问主存的地址
- **存储器数据寄存器MDR**：存放中粗粒写入或读出的数据
### 3.3 数据通路的构成
#### 3.3.1 基于单总线的数据通路
自己回去看王道补
#### 3.3.2 基于双总线的数据通路
#### 3.3.3 基于专用通路结构的数据通路
#### 3.3.4 单周期和多周期处理器数据通路
- **单周期处理器**：所有的指令在一个时钟周期内完成的处理器
- **多周期处理器**：指令可根据执行的时间长度，在一个时钟周期或多个时钟周期内完成的处理器
### 3.4 
## 4. 指令执行过程
### 4.1 指令周期
- **指令周期**：指的是CPU取指并执行所需的全部时间
- **机器周期**：指令周期由若干机器周期组成，机器周期也叫**CPU周期**
- **时钟周期**：一个机器周期包含若干时钟周期(节拍，T周期，或者CPU时钟周期，最基本单位)
如图  
![alt text](image/指令周期.png)
#### 4.1.1 主频
例如主频 = 3.0 GHZ，表示每秒能完成3.0 G个时钟周期
#### 4.1.2 机器周期
像取指，计算地址，取数，传递结果对应一个机器周期
## 5. 组合逻辑控制器
## 6. 微程序控制器
### 6.1 微程序，微指令，微操作
- **程序**：由指令序列组成
- **微程序**：由微指令序列组成，一个指令对应一个微程序
- **微指令**：同时发出的控制信号所执行的一组微操作称为微指令
- **微操作**：指的是一个指令中所需要的操作
![alt text](image/微程序.png)
### 6.2 微指令基本格式
![alt text](image/微指令基本格式.png)  
- **操作控制**：表示要进行哪些微操作
- **顺序控制**：表示下一条要进行的微操作地址  
微指令存在与控制器的特定的寄存器下
### 6.3 微程序控制器的基本结构
执行一条指令实际上就是执行一段存放在控制存储器中的微程序。  
![alt text](image/微程序控制器的基本结构.png)
### 6.4 微程序设计技术
#### 6.4.1 水平型微指令
##### 6.4.1.1 直接控制法
在微指令的控制字段中，每一位代表一个微命令，根据是否发出某个微命令，设置对应比特位成“1”或“0”，这样就可打开或关闭某个控制门。就相当于开关。  
![alt text](image/直接控制法add指令序列.png)  
![alt text](image/直接控制法add指令序列的编字.png)  
因此，根据add所需的微操作就编写出了对应的微指令。
##### 6.4.1.2 字段直接编译法
有些微操作是互斥的，在一个微指令中只可能出现其中一个微操作。因此我们用同一个字段编译这一组微操作  
![alt text](image/字段直接编译法add的指令分段.png)
![alt text](image/字段直接编译法add的指令序列编字.png)  
很好理解的，为了能互斥，设计时要遵循三个原则：
- **分段原则**：分段要按相容性和相斥性原则进行。
- **相容性原则**：可以在同一时刻发出的微命令分在不同组内。
- **相斥性原则**：不能在同一时刻发出的微命令分在同一组内。  
而且，很重要的一点：因为任何一组微操作都有可能同时不出现，因此还要留一位给全不出现的微操作
##### 6.4.1.3 字段间接编译法
在字段直接编译法的基础上，进一步缩短微指令字长的一种编译法。  
有些段的微操作，需要别的段的微操作来确认。  
![alt text](image/字段间接编译法.png)
##### 6.4.1.4 常数源字段E
微指令会设置有常数源字段E，类似于指令中的立即数，有时被称为 **"发射字段"**
#### 6.4.2 垂直型微指令
一个微指令只完成一个微操作
#### 6.4.3 混合型微指令
水平和垂直的混合。
...
![alt text](image/增量产生微指令下地址.png)
#### 6.5.2 增量与下址字段结合产生后继微地址
就是说，微指令的下址字段分成两部分
- **转移控制字段BCF**
- **转移地址字段BAF**  
若BCF符合，则BAF送到μPC，否则μPC+1送到μPC
...
- **替换控制位**：指出需替换的页。
#### 8.2.4 快表
![alt text](image/快表.png)  
把**页表的最活跃部分**存放在快速存储器中组成快表，这是减少时间开销的一种方法。
- **快表**，是一个快速存储器，支持快速的搜索。  
- **快表作用**：慢表(页表)存储在主存，速度慢，快表不在主存，很快能查。先查快表不行再查慢表，加快速度。
#### 8.2.5 页这种方式的优缺点
- 优点：充分利用主存空间
...
### 8.5 虚拟存储器工作的全过程
- 程序员的程序会给出**多用户虚地址**，通过页表进行内部地址变换，将**页号**转为**页面号**。
- 如果通过控制字段发现内容已经在主存中，就去主存操作，执行3。如果不在主存中，则进行4和5。
- 通过外部地址变换得到辅存的实地址。如果得到的地址不存在，就会**中断**，进行8。否则访问辅存。
- 先将辅存内容放入I/O处理机，拿出主存页表，假如没装满，则进行10写入映射。否则进行11利用**替换算法**进行页面置换。
- 在写入主存之前，要通过控制字段知道被替换页面是否经过修改，要是被修改了，执行14写回辅存保持一致性；否则，不做操作。  
![alt text](image/虚拟存储器工作的全过程.png)
