...
if __name__ == '__main__':
    ...
    times = 4 if hour < 21 else 24 - hour 
    ...
    for i in range(times):  # 连续调度3个小时
        now_hour = hour + i
        df = get_data(date, now_hour, df_all)
        ...
        for _, row in df.iterrows():
            ...
            # 添加映射后的 distance_x 和 distance_y
            record = {
            'inflow': row['inflow'],
            'outflow': row['outflow'],
        ...
        # 假设 records, distance_x, distance_y 都已经准备好了
        for record, x, y in zip(records, distance_x, distance_y):
            record['x'] = x
            record['y'] = y
        
        records_df = pd.DataFrame(records)
        records_df.to_csv(f"records_{now_hour}.csv", index=False)
        ...
        # 更新库存
        if tsp.best_ant:
            for idx in tsp.best_ant.path:
                coord = grid_coords[idx]
                if bi[idx] < 0:
                    inventory[coord] = inventory.get(coord, 0) + abs(bi[idx])  # 卸车
                elif bi[idx] > 0:
                    inventory[coord] = inventory.get(coord, 0) - bi[idx]       # 装车
            print(" 调度完成，库存已更新。")
        else:
            print(" 未找到有效路径，跳过库存更新。")
