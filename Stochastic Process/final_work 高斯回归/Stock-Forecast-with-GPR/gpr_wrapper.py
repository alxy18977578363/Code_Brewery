...
class Wrapper:
    ...
    def __init__(self, company_name: str):
        ...
        # 协方差函数：RationalQuadratic + Matern + WhiteKernel
        from sklearn.gaussian_process.kernels import RationalQuadratic, Matern, WhiteKernel
        kernel = RationalQuadratic(length_scale=1.0, alpha=1.0, length_scale_bounds=(0.1, 100), alpha_bounds=(0.1, 10.0))
        kernel += Matern(length_scale=1.0, length_scale_bounds=(0.1, 100), nu=1.5)
        kernel += WhiteKernel(noise_level=1.0, noise_level_bounds=(1e-3, 1e+1))

        self.__iterations = 20
        self.__alpha = 1e-6
        self.__kernels = [kernel]
        self.__gp = GaussianProcessRegressor(
            kernel=self.__kernels[0],
            alpha=self.__alpha,
    ...
    def get_eval_model(self,
        ...
        if start_year == self.__company_data.years[0]:
            ...
            first_year_prices = pd.concat([pd.Series([0.0], index=[first_year_prices.index[0]-1]), first_year_prices])
